...
type FormAction =
  ...
  | { type: 'RESET'; initialData: TaskFormInitialData | null; targetStatus: TaskStatus };
...
function formReducer(state: FormState, action: FormAction): FormState {
  switch (action.type) {
    ...
    case 'RESET':
      return {
        title:       action.initialData?.title ?? '',
        description: action.initialData?.description ?? '',
        status:      action.initialData?.status ?? action.targetStatus,
        errors:      {},
        loading:     false,
      };
  }
...
export function TaskFormModal({ isOpen, mode, initialData, targetStatus, onSubmit, onClose }: TaskFormModalProps) {
  ...
  useEffect(() => {
    if (!isOpen) return;
    dispatch({ type: 'RESET', initialData, targetStatus });
    titleRef.current?.focus();
  }, [isOpen, initialData, targetStatus]);
